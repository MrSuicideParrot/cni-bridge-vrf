...
func loadNetConf(bytes []byte, envArgs string) (*NetConf, string, error) {
	...
	if n.VRFName == "" {
		return nil, "", fmt.Errorf("configuration is expected to have a valid vrf name")
	}
...
func cmdAdd(args *skel.CmdArgs) error {
	...
	if err == nil && n.Table != 0 && vrf.Table != n.Table {
		return fmt.Errorf("VRF %s already exist with different routing table %d", n.VRFName, vrf.Table)
	}

	if _, ok := err.(netlink.LinkNotFoundError); ok {
		vrf, err = createVRF(n.VRFName, n.Table)
	}
	...
	err = addInterface(vrf, brInterface.Name)
	if err != nil {
		return err
	}
...
func cmdCheck(args *skel.CmdArgs) error {
	...
	// Check if bridge has the master vrf defined
	vrfInterfaces, err := assignedInterfaces(vrf)

	found := false
	for _, intf := range vrfInterfaces {
		if intf.Attrs().Name == brCNI.Name {
			found = true
			break
		}
	}
	if !found {
		return fmt.Errorf("Failed to find %s associated to vrf %s", args.IfName, n.VRFName)
	}
